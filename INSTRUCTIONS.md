...
1. **Start with brand** - We know the brand (e.g., "METRO")
...
8. **Scrape new jobs** - Extract jobs from METRO careers page (Romania)
...
3. **Scrape jobs** - Extract jobs from METRO careers API (Romania only)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape METRO API (jobs for Romania)
    │
    ▼
...
| `tests/e2e/scraper.test.js` | End-to-end tests with real METRO API |
